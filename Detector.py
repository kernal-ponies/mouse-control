...
class CascadeDetector:
    def __init__(self):
        ...
        self.PUPIL_THRESH = cv2.getTrackbarPos("Eye Detection Threshold", WINDOW_NAME)
    ...
    def detectPupils(self, frame, threshold=127):
        img = cv2.copyTo(frame, None)
    ...
    def check_eyes(self):
        if cv2.norm(np.array(self.tmp_right_pupil, np.int32), np.array(self.right_pupil, np.int32)) > self.move_thresh \
                and cv2.norm(np.array(self.tmp_left_pupil, np.int32), np.array(self.left_pupil, np.int32)) > self.move_thresh:
        ...
        for (x, y, w, h) in faces:
            ...
            for (ex, ey, ew, eh) in eyes:
                ...
                if ex + ew /2 < face_w /2:
                    ...
                    ex, ey, ew, eh, self.previous_left_eye = self.stabilize_eyes_frame(face_x, face_y, ex, ey, ew, eh, self.previous_left_eye)
                    if self.phase>0:
                    ...
                    self.lp_frame = cv2.drawKeypoints(self.lp_frame, lp_keypoint, self.lp_frame, (0, 255, 0), cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)

                    # if pupil is detected, then do something,
                    # otherwise, just consider center of eye as pupil
                    if len(lp_keypoint) > 0:
                        ...
                        self.face_frame = cv2.circle(self.face_frame, (ex + self.left_pupil[0], ey + self.left_pupil[1]), 5, (0, 255, 0), 4)
                    else:
                        self.tmp_left_pupil = [face_x + ex + int(ew / 2), face_y + ey + int(eh / 2)]
                # Right eye
                else:
                    self.right_is_visible = True
                    ex, ey, ew, eh, self.previous_right_eye = self.stabilize_eyes_frame(face_x, face_y, ex, ey, ew, eh, self.previous_right_eye)
                    ...
                    self.rp_frame = cv2.drawKeypoints(self.rp_frame, rp_keypoint, self.rp_frame, (0, 255, 0), cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
                    ...
                    if len(rp_keypoint) > 0:
                        ...
                        self.face_frame = cv2.circle(self.face_frame, (ex + self.right_pupil[0], ey + self.right_pupil[1]), 5, (0, 255, 0), 4)
                    else:
